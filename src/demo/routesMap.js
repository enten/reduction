...
export default {
  ...
  LIST: {
    ...
    thunk: async (dispatch, getState) => {
      ...
      const videos = await fetchData(`/demo/videos/${category}`, jwToken)
  ...
  VIDEO: {
    ...
    thunk: async (dispatch, getState) => {
      ...
      // using fetchData(`/demo/video/${slug}`) and by dispatching
  ...
  ADMIN: {
    ...
    role: 'admin' // + change req.cookies.jwt to 'real' in server.js
  },
  FOO: {
    path: '/foo'
  }
...
const video = await fetchData(`/demo/video/${slug}`, jwToken)
