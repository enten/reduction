...
export function middleware () {
  ...
  router.get('/:category', async (req, res) => {
    const data = await findVideos(req.params.category, req.jwt)
    res.json(data)
  })

  router.get('/:slug', async (req, res) => {
    const data = await findVideo(req.params.slug, req.jwt)
    res.json(data)
  })
...
export async function findVideos (category, jwToken) {
...
export async function findVideo (slug, jwToken) {
  await fakeDelay(500)
